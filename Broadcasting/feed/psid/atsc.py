...
tvct = terrestrial_virtual_channel_section(
	transport_stream_id = feed_transport_stream_id,
	ATSC_protocol_version = 0,
	channels_loop = [
		channel_loop_item(
			short_name = "\x00\x46\x00\x45\x00\x45\x00\x44\x00\x2D\x00\x44\x00\x54", # FEED-DT as utf-16
			major_channel_number = 68,
			minor_channel_number = 1,
			modulation_mode = 0x04, #8 VSB
			carrier_frequency = 0, # deprecated
			channel_TSID = feed_transport_stream_id,
			program_number = feed1_service_id,
...
pmt = program_map_section(
	program_number = feed1_service_id,
	PCR_PID = feed1_pmt_pid + 1,
	program_info_descriptor_loop = [],
	stream_loop = [
		stream_loop_item(
			stream_type = 2, # mpeg2 video stream type
			elementary_PID = feed1_pmt_pid + 1,
			element_info_descriptor_loop = [
                                #registration_descriptor(
				#	format_identifier="GA94", 
                                #),
			]
		),
                stream_loop_item(
                        stream_type = 129, # ac3 audio stream type
                        elementary_PID = feed1_pmt_pid + 3,
                        element_info_descriptor_loop = [
                                registration_descriptor(
...
out.close
os.system('/usr/bin/sec2ts ' + str(feed1_pmt_pid) + ' < ./pmt.sec > ./pmt.ts')
...
out.close
os.system('/usr/bin/sec2ts 8187 < ./mgt.sec > ./mgt.ts')
...
out.close
os.system('/usr/bin/sec2ts 8187 < ./tvct.sec > ./tvct.ts')
