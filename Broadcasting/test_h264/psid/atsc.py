...
avalpa_transport_stream_id = 0 # demo value, an official value should be demanded to dvb org
...
tvct = terrestrial_virtual_channel_section(
	...
	channels_loop = [
		channel_loop_item(
			short_name = "\x00\x54\x00\x45\x00\x53\x00\x54\x00\x2D\x00\x44\x00\x54", # TEST-DT as utf-16
			major_channel_number = 69,
...
pmt = program_map_section(
	...
	stream_loop = [
		stream_loop_item(
			stream_type = 27, # h264 video stream type
			elementary_PID = 65,
...
out.close
os.system('/usr/bin/sec2ts ' + str(avalpa1_pmt_pid) + ' < ./pmt.sec > ./pmt.ts')
