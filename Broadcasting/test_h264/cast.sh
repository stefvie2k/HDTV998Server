...
echo "Killall command after Ctrl-C is: killall esaudio2pes pesaudio2ts tsloop esvideompeg2pes pesvideo2ts tsstamp tscbrmuxer"
...
ffmpeg -v 0 -i audio.wav -vn -acodec ac3 -ac 1 -ab 128k -ar $audio_sample_rate -f ac3 -y audio.es
...
# -ab 256k		audio bit rate in kbps
...
ffmpeg -v 0 -re -f lavfi -i "smptehdbars=size=1920x1080:rate=$video_frame_rate" \
...
x264 --profile baseline --level 4.0 --nal-hrd cbr --vbv-bufsize 200 --bitrate 720 --keyint 12 --vbv-init 0 -o video.pes video.y4m &
...
pesvideo2ts $video_pid $video_frame_rate b720000 760000 0 video.pes > video.ts &
...
# b:12463634		null bit rate
# null.ts		null transport stream ... this is require to make sure we meat the ATSC standard of 19.39Mbps
